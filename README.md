...
- Version 1.5 : affichage court par défaut : une seule action immédiate et une phrase de contexte. `/mc details` affiche les informations longues uniquement à la demande. Le joueur peut choisir `discovery`, `support` ou `progression` avec `/mc mode <nom>`.
